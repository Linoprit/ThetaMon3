...
class AppSettings:
    ...
    def _command_line_init(self):
        ...
        self.arg_parser.add_argument(
            'command', action='store', help=textwrap.dedent('''\
                Allowed commands are:
                'parse' - reads an ID-file and converts it.'''))
        self.arg_parser.add_argument(
            '-f', '--file', help='''File to read in.''')
        self.arg_parser.add_argument(
            '-o', '--output', help='''Output file.''')
        self.arg_parser.add_argument(
            '-w', '--workdir', help=textwrap.dedent('''\
                Path to the working directory, named 'data', where
                the subdirs 'input', 'output' and 'temp' reside.
                Will be created, if not in existence.'''))
        self.arg_parser.add_argument(
            '-s', '--settingsFile', help=textwrap.dedent('''\
                settings file to use, without path.
    ...
    def __overlay_settings(self, args):
        ...
        if args.command:
            self.settings['common']['command'] = args.command
        if args.file:
            self.settings['table_id_tool']['raw_id_file'] = args.file
        if args.output:
            self.settings['table_id_tool']['id_table_outfile'] = args.output
