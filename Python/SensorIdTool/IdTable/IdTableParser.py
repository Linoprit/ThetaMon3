import logging
from IdTable.SensorId import SensorId as SensorId
        ...
        for line in file_content:
            ...
            if not act_item.set_sensor_ids_from_hex(sens_id):
                logging.getLogger().error(
                    "Length of sensor-id wrong in line " + str(line_count+1))
            if not act_item.set_sensor_ids64_from_hex(sens_id):
                logging.getLogger().error(
                    "Creating uint64 from sensor-id went wrong. line " + str(line_count+1))
            sens_type = splitted[self.POS_TYPE]
            if not act_item.set_sensor_type(sens_type):
                logging.getLogger().error(
                    "unrecognized sensor-type [0, 3]/[temp, hum, pres, rel] in line "
                    + str(line_count+1))
            min_val = float(splitted[self.POS_MIN])
            if not act_item.set_min_val(min_val, sens_type):
                logging.getLogger().error(
                    "min_value out of range in line " + str(line_count+1))
            max_val = float(splitted[self.POS_MAX])
            if not act_item.set_max_val(max_val, sens_type):
                logging.getLogger().error(
                    "max_value out of range in line " + str(line_count+1))
            relay = int(splitted[self.POS_RELAY])
            if not act_item.set_relay(relay):
                logging.getLogger().error(
                    "relay value out of range [0:255] in line " + str(line_count+1))
            shortname = splitted[self.POS_SHORTNAME].strip()
            if not act_item.set_shortname(shortname):
                logging.getLogger().error(
                    "shortname too long [max 8] in line " + str(line_count+1))
