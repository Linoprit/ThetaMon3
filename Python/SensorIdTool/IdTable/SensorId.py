...
from IdTable.calcStm32Crc import CalcStm32Crc as Calc32
import struct
...
class SensorId:
    ...
    sensor_id_hash = np.uint32  # []  # list[np.uint32], len = 2
    sensor_id_u64 = np.uint64
    ...
    def set_sensor_ids_from_hex(self, hex_list):  # hex_list: eight hex strings
        ...
        self.sensor_id_u64 = self.u64_id_from_str(hex_list)
        return True

    def set_sensor_ids64_from_hex(self, hex_list):  # hex_list: eight hex strings
        if len(hex_list) != 8:
            return False
        byte_list = [np.uint8(int(x, 16)) for x in hex_list]
        self.sensor_id = byte_list
    ...
    @staticmethod
    def u64_id_from_str(hex_list) -> np.uint64:
        byte_list = [np.uint8(int(x, 16)) for x in hex_list]
        list_as_array = bytearray(byte_list)
        result = struct.unpack("<Q", list_as_array)[0]
        return result
    ...
    def set_sensor_type(self, sensor_type: str):
        ...
        elif sens_str.startswith("relay") or sens_str == '3':
            self.sensor_type = SensorType.RELAY
        else:
            return False
        return True
    ...
    def check_range_min_max(value: float, sensor_enum: SensorType):
        if sensor_enum == SensorType.TEMP:
            ...
            if value < 846.0 or value > 1100.0:
                return False
        elif sensor_enum == SensorType.RELAY:
