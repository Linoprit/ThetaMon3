...
public:
	OneWire();
	OneWire(uint8_t GPIO_TX_Pin, uint8_t GPIO_RX_Pin);
	virtual ~OneWire() {
	}
	;
	...
	inline void owDelay(uint32_t time_us) {
	...
	inline void lineLow(void) {
		...
		digitalWrite(_GPIO_TX_Pin, LOW);   
		//HAL_GPIO_WritePin(_GPIO_TX_PORT, _GPIO_TX_Pin, GPIO_PIN_RESET);
	#else
		digitalWrite(_GPIO_TX_Pin, HIGH);   
...
#if (OW_LINE_INVERTED == true)
	return digitalRead(_GPIO_RX_Pin);
...
private:
	...
	uint8_t _GPIO_TX_Pin;
	//GPIO_TypeDef *_GPIO_RX_PORT;
	uint8_t _GPIO_RX_Pin;
