...
char tokenBuff[16]; // UInt64Token is the largest one, which must fit in here

Lexer::Lexer(CmdBufferType *comLine) { setComLine(comLine); }
...
Token *Lexer::makeNum(void) {
  ...
  if ((isInteger) && (isSigned == false)) {
    uint64_t result = std::stoull(reinterpret_cast<char *>(startChar), &sz);
    return new (tokenBuff) UInt64Token(result);
  } else if ((isInteger) && (isSigned)) {
    int32_t result = std::stol(reinterpret_cast<char *>(startChar), &sz);
    return new (tokenBuff) SInt32Token(result);
  }
...
uint32_t Lexer::makeCmd(void) {
  ...
  hashCode =
      CrcSocket::calcChksum32(reinterpret_cast<uint32_t *>(commandBuf), len32);
