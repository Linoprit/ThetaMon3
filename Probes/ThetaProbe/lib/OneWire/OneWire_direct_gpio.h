...
#define DIRECT_WRITE_HIGH(base, mask)    (mask > 0x8000) ? GP16O &= ~1 : (GPOC = (mask))
#define DIRECT_WRITE_LOW(base, mask)   (mask > 0x8000) ? GP16O |= 1 : (GPOS = (mask))
...
#define DIRECT_WRITE_LOW(base, pin)     digitalWriteFast((PinName)pin, HIGH)
#define DIRECT_WRITE_HIGH(base, pin)    digitalWriteFast((PinName)pin, LOW)
...
#define DIRECT_WRITE_HIGH(base, mask)    ((*((base)+5)) = (mask))
#define DIRECT_WRITE_LOW(base, mask)   ((*((base)+6)) = (mask))
...
#define DIRECT_WRITE_HIGH(base, pin)     CY_SYS_PINS_CLEAR_PIN(PORT_ADDRESS(pin), PIN_IN_PORT(pin))
#define DIRECT_WRITE_LOW(base, pin)    CY_SYS_PINS_SET_PIN(PORT_ADDRESS(pin), PIN_IN_PORT(pin))
...
#define DIRECT_WRITE_HIGH(base, pin)     nrf_gpio_pin_clear(pin)
#define DIRECT_WRITE_LOW(base, pin)    nrf_gpio_pin_set(pin)
...
{
    return ((GPIO_REG(GPIO_INPUT_VAL) & mask) != 0) ? 0 : 1;
}
...
#define DIRECT_WRITE_LOW(base, pin)     (*pin = 1)
#define DIRECT_WRITE_HIGH(base, pin)    (*pin = 0)
