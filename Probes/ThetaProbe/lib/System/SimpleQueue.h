 ...
 * 	if (!_keyBuffer.isEmpty()) { chr = _keyBuffer.dequeue(); }
 * Push an element to back of queue:
 * 	if (!_keyBuffer.isFull()) { _keyBuffer.enque(chr); }
 * Returns all elements as array, w/o dequeueing.
 * 	asArray(T* data)
 ...
 * 	bool isEmpty()
 * 	bool isFull()
 * Returns first element w/o dequeueing. 
 * 	T front()
 * Counter, in which position the next element is put, or taken
 * 	int16_t getFront()
 * 	int16_t getRear()
 */
...
public:
	...
	inline void enqueue(T element) { // put element to back of queue
	...
	inline T dequeue(void) { // return element and remove it from queue
		if ( isEmpty() ) {
			error_handler(__FILE__, __LINE__ );
		}
	...
	bool isFull(void)  { return (static_cast<std::size_t>(_size) == _maxSize); };
